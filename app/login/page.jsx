    ...
    try {
      ...
      if (err && typeof err === 'object') {
        ...
        if (msg.includes('auth/popup-closed-by-user')) {
          ...
          errMsg = 'This domain is not authorized for Google Sign-In. Please check your Firebase console settings.';
        } else if (msg.includes('auth/operation-not-allowed')) {
          errMsg = 'Google Sign-In is not enabled as a provider in your Firebase project. Please enable it in the console.';
        } else {
          errMsg = msg;
        }
      }
        ...
        <div className="w-full max-w-md bg-white border border-slate-200/80 p-8 rounded-3xl shadow-xl shadow-slate-100/40">
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              disabled={submitting || googleLoading}
              className="group relative w-full py-2.5 px-4 bg-white hover:bg-slate-50 active:bg-slate-100 text-slate-800 border border-slate-200 rounded-xl text-xs font-bold transition flex items-center justify-center gap-3 shadow-sm disabled:opacity-60 cursor-pointer"
            >
              {googleLoading ? (
